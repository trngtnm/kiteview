...
{
  NSString *next =
      [colorScheme isEqualToString:@"light"] ? @"light" : @"dark";
  if ([_colorScheme isEqualToString:next]) {
    return;
  }
  _colorScheme = [next copy];
  [self kv_applyColorScheme];
}

- (void)kv_applyColorScheme
{
  BOOL light = [self.colorScheme isEqualToString:@"light"];
  if (@available(macOS 10.14, *)) {
    self.effectView.appearance = [NSAppearance
        appearanceNamed:light ? NSAppearanceNameAqua : NSAppearanceNameDarkAqua];
  }
  if (light) {
    self.effectView.material = NSVisualEffectMaterialPopover;
    self.tintView.layer.backgroundColor =
        [NSColor colorWithCalibratedWhite:1.0 alpha:0.22].CGColor;
  } else {
